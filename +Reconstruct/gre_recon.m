...
if strcmp(hdr.acquisitionSystemInformation.institutionName,'Iowa')
    recon_image = zeros(rec_Nx,rec_Ny/2,nContrasts,nSlices);
else
    ...
    for contrast = 1:nContrasts
        for slice = 1:nSlices
            ...
            try
                ...
                K = zeros(enc_Ny, enc_Ny, enc_Nz, nCoils);
                % Select the appropriate measurements from the data
                acqs = find(  (meas.head.idx.contrast==(contrast-1)) ...
                        & (meas.head.idx.repetition==(rep-1)) ...
                        & (meas.head.idx.slice==(slice-1)));
            ...
            try
                ...
                mytest(:,:,contrast,slice) = K;
            catch
                K_space = zeros(enc_Ny,enc_Ny*2,enc_Nz,nContrasts,nSlices,nCoils);
                myim = zeros(rec_Ny,rec_Ny,rec_Nz,nCoils);
                ...
                myim((cenx-col_center+1):(cenx-col_center+enc_Ny),(ceny-phs_center+1):(ceny-phs_center+enc_Ny),:,:) = K;
                K_space(:,(ceny-phs_center+1):(ceny-phs_center+enc_Ny),:,contrast,slice,:) = K;
                mytest(:,:,contrast,slice) = K;
            end
            ...
            try
                ...
                recon_image(:,:,contrast,slice) = myim((rec_Nx/4+1):(3*rec_Nx/4),1:size(recon_image,2));
            end
        end
    end
end
