function [gas_hires,gas_lores,dis,gas_k,dis_k] = xpdixon_recon(filename,delay_cor,nii_write)
%%
if nargin < 2
    delay_cor = 0;
    nii_write = true;
...
figure('Name','Gas_Image_sharp_recon');
montage(abs(squeeze(SharpIms(:,:,:,1))))
maxim = abs(squeeze(SharpIms(:,:,:,1)));
maxim = max(maxim(:));
clim([0 0.8*maxim])
...
if nii_write
    ...
    writesgas = ReadData.mat2canon(abs(SharpIms(:,:,:,1)));
    writebgas = ReadData.mat2canon(abs(BroadIms(:,:,:,1)));
    writedis = ReadData.mat2canon(abs(BroadIms(:,:,:,3)));
