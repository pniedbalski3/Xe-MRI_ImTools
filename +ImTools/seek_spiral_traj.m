...
all_traj = dir(fullfile(parent_path,'Traj_Files'));
...
if ~isempty(all_traj)
    ...
    if isnan(proper_traj)
        for i = 1:length(all_traj)
            traj_twix = DataImport.mapVBVD(fullfile(parent_path,'Traj_Files',all_traj(i).name));
...
if ~isnan(proper_traj)
    traj_twix = DataImport.mapVBVD(fullfile(parent_path,'Traj_Files',all_traj(proper_traj).name));
